...
public class Interfaz {
	...
	private void initialize() {
		...
		JLabel lblNewLabel_2_1_1_1 = new JLabel("Grupos");
		...
		JButton btn_grafo = new JButton("Clusters");
		btn_grafo.addActionListener(new ActionListener() {
			...
			private void cargarListas() {
				if(aplicacion.getPersonas().size() >= 2) {
					
					aplicacion.crearAristas();
					
					aplicacion.crearCluster();
					
	                ArrayList<Vertice> c1 = aplicacion.getGrupo1();
	                ArrayList<Vertice> c2 = aplicacion.getGrupo2();
	
	                for(Vertice v : c1) {
	                    Object[] info = new Object[] {v.getPersona().getNombre(), v.getPersona().getD(),
	                    		v.getPersona().getM(), v.getPersona().getE(), v.getPersona().getC()};
						 
						 ((DefaultTableModel) lista1.getModel()).addRow(info);
	                }
	                for(Vertice v : c2) {
	                	Object[] info = new Object[] {v.getPersona().getNombre(), v.getPersona().getD(),
	                    		v.getPersona().getM(), v.getPersona().getE(), v.getPersona().getC()};
	                	
						 ((DefaultTableModel) lista2.getModel()).addRow(info);
	                }
				}
			}
		});
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Nombre", "Deporte", "M�sica", "Espect�culo", "Ciencia"
			}
		));
